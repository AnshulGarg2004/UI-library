        ...
        <AnimatePresence>
            ...
            >
                ...
                >
                    ...
                    <button className='absolute top-3 right-3 z-20 w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-all cursor-pointer' onClick={onClose}>
                        <TbX size={15} />
                    </button>

                    <div className='sm:w-[52%] bg-gradient-to-r from-[#03181c] to-[#041e24] p-6 sm:p-10 relative overflow-hidden'>
                        ...
                        <motion.div
                            ...
                            <div className='w-9 h-9 rounded-xl bg-gradient-to-br from-[#3be8ff] to-[#0ab5d4] flex items-center justify-center shadow-[0_0_18px_rgba(59,232,255,0.35)]'><SiValorant size={17} color='#051c20' /></div>
                            <span className='text-xl font-bold text-[#e8f8fa] tracking-tight'
                                style={{ fontFamily: '"Syne", "san-serif"' }}>Virtual UI</span>
                        </motion.div>
                        ...
                        <div className='flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1'>
                            {steps.map((step, index) => (
                                <motion.div
                                    ...
                                    className={`flex-shrink-0 sm:flex-shrink flex items-start gap-3 px-3 py-2.5 rounded-xl border transition-all duration-300 min-w-[200px] sm:min-w-0 cursor-pointer ${active === index ? 'bg-[#3be8ff]/[0.07] border-[#3be8ff]/20' : 'bg-transparent border-transparent'}`}
                                >
                                    <div className={`min-w-[28px] h-7 rounded-lg flex items-center justify-center border transition-all duration-300 ${active === index ? 'bg-gradient-to-br from-[#3be8ff] to-[#0ab8d6] border-transparent' : 'bg-[#3be8ff]/[0.08] border-[#3be8ff]/20'}`}>
                                        <step.icon size={20} color={active === index ? '#051c20' : '#3be8ff'} />
                                    </div>

                                    <div>
                                        <p className={`text-[12.5px] font-semibold transition-colors duration-300 whitespace-nowrap sm:whitespace-normal ${active === index ? 'text-[#d4f5fa]' : 'text-white/55'}`}>
                                            {step.title}
                                        </p>

                                        <div className={`overflow-hidden transition-all duration-500 ${active === index ? 'max-h-8 opacity-100 mt-0.5' : 'max-h-0 opacity-0'}`}>
                                            <p className='text-[9px] text-[#3be8ff]/40 leading-relaxed '>
                                                {step.desc}
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <motion.div
                        ...
                        <div className='relative z-20 w-full max-w-[280px] sm:max-w-[260px] text-center mx-auto'>
                            <motion.div
                                animate={{ y: [0 - 6, 0] }}
                            ...
                            <p className='text-[13px] text-[#96bec8]/55 mb-6 leading-relaxed sm:mb-7'>
                                Sign in to generate AI powered UI components in seconds</p>

                            <div className='flex justify-center gap-4 sm:gap-5 mb-6 sm:mb-7'>
                                {
                                    [["200 Credits"], ["∞ Components"]].map(([v, l], i) => (
                                        <div key={i} className='text-center'>
                                            <div className='text-base font-bold text-[#3be8ff]'>{v}</div>
                                            <p className='text-[9px] text-[#78aab4]/45 uppercase tracking-wider font-medium'>{l}</p>
                            ...
                            <p className='text-[11px] text-[#64919b]/45 mt-4 sm:mt-5'>No account needed for npm.{" "}
