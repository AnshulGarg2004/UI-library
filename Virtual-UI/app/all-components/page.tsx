...
import GuidedPanel from '../components/guide-panel';
...
const AllComponents = () => {
    ...
    const publicComponents = components.filter((compo) =>
    compo.visibility === "public").filter((compo) => compo.name.toLowerCase().includes(search.toLowerCase())).sort((a,b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
        ...
        <div className="min-h-screen bg-[#030b0d] text-white flex flex-col overflow-hidden"
            ...
            <div className='flex flex-1 min-h-0'>
                <aside className='hidden sm:block w-72'>
                    <SidebarComponent selected={selected} search={search} publicComponents={publicComponents} onSelect={handleSelect} setSearch={setSearch}/>
                </aside>
                       ...
                       <>
                            ...
                            >
                               ...
                               onClick={() => setSidebarOpen(false)} 
                                ...
                                <SidebarComponent selected={selected} search={search} setSearch={setSearch} publicComponents={publicComponents} onSelect={handleSelect} />
                            </motion.div>
                       </>
                    )}
                </AnimatePresence>
